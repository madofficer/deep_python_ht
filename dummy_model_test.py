...
class TestModel(unittest.TestCase):
    ...
    @patch('dummy_model.DummyModel.predict')
    def test_predict_message_mood_invalid_thresholds_type(self, mock_prediction):
        mock_prediction.return_value = 0.5
        with self.assertRaises(TypeError):
    ...
    @patch('dummy_model.DummyModel.predict')
    def test_predict_message_invalid_good_thresholds_value(self, mock_prediction):
        mock_prediction.return_value = 0.5
        with self.assertRaises(ValueError):
            predict_message_mood("Вулкан", 0.5, 1.1)

    @patch('dummy_model.DummyModel.predict')
    def test_predict_message_invalid_bad_thresholds_value(self, mock_prediction):
        mock_prediction.return_value = 0.5
        with self.assertRaises(ValueError):
