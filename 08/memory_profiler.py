...
def modify(instances):
    for obj in instances:
        obj.attr1, obj.attr2, obj.attr3, = obj.attr2, obj.attr3, obj.attr1
...
for name, Class in classes.items():
    time = timeit.timeit(lambda: create_obj(Class, n), number=10)
    print(f'{name}: create {n} examples - {time:.2f} sec')


for name, Class in classes.items():
    instances = create_obj(Class, n)
    time = timeit.timeit(lambda: modify(instances), number=10)
